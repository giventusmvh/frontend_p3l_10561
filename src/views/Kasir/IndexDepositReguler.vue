<template>
  ...
  <div class="container d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">List Transaksi Deposit Uang Reguler</h1>
  </div>
  <div class="mt-5">
    <div class="row">
      <div class="col-md-12">
            ...
            <table class="table table-striped table-bordered mt-4 table-responsive shadow">
              <thead class="thead-dark">
                <tr class="text-center">
                  ...
                  <th scope="col">TANGGAL TRANSAKSI UANG</th>
              ...
              <tbody class="text-center">
                <tr v-for="(uang, id) in uangs" :key="id">
                  <td>{{ uang.no_struk_uang }}</td>
                  <td>{{ uang.nama_member }}</td>
                  <td>{{ uang.tgl_TransaksiUang }}</td>
                  <td>{{ uang.jumlah_bayar_uang }}</td>
                  <td>{{ uang.bonus_deposit_uang }}</td>
                  <td>{{ uang.total_deposit_uang }}</td>
                  <td>{{ uang.sisa_deposit_uang }}</td>

                  <td class="text-center">
                    <button class="btn btn-sm btn-success m-1" @click="generatePdf(uang.id, uang.id_member_uang)"><i class="bi bi-filetype-pdf"></i></button>
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  setup() {
    ...
    onMounted(() => {
      ...
      axios
        .get("https://api.gofit.given.website/api/transaksiUang")
        .then((response) => {
          //assign state posts with response data
          uangs.value = response.data.data;
        })
        .catch((error) => {
    ...
    function generatePdf(id1, id2) {
      ...
      axios.defaults.headers.common["Authorization"] = `Bearer ${token}`;
      axios
        .all([axios.get(`https://api.gofit.given.website/api/transaksiUang/${id1}`, {}), axios.get(`https://api.gofit.given.website/api/user/${id2}`, {}), axios.get(`https://api.gofit.given.website/api/pegawai/${id3}`, {})])
        .then(
          axios.spread((res1, res2, res3) => {
            this.uang = res1.data.data;
            ...
            doc.text("---STRUK DEPOSIT REGULER---", cardX + 25, cardY + 7);
            doc.text("No Struk: " + this.uang.no_struk_uang, cardX + 5, cardY + 15);
            doc.text("Nama Member / ID Member: " + this.member.nama_member + "/" + this.member.id_member, cardX + 5, cardY + 25);
            doc.text("Tanggal: " + this.uang.tgl_TransaksiUang, cardX + 5, cardY + 35);
            doc.text("Deposit: " + this.uang.jumlah_bayar_uang, cardX + 5, cardY + 45);
            doc.text("Bonus Deposit: " + this.uang.bonus_deposit_uang, cardX + 5, cardY + 55);
            doc.text("Sisa Deposit: " + this.uang.sisa_deposit_uang, cardX + 5, cardY + 65);
            doc.text("Total Deposit: " + this.uang.total_deposit_uang, cardX + 5, cardY + 75);
            doc.text("Kasir: " + this.pegawai.nama_pegawai + "/" + this.pegawai.id_pegawai, cardX + 5, cardY + 90);

            // Download the PDF
            doc.save("strukDepositReguler.pdf");
          })
        )
        .catch((error) => {
          console.log(error);
